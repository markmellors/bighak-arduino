    ...
    if (digitalRead(RCSwitchPin)){
      ...
      if (Serial.available() > 0) { //check if a command has been received
        ...
        if (NoCommand==false){  //if nocommand==false then this is the first break in recent times
          ...
          if(millis()-LastCommand>200){  //if its been a while
            LaserOn=false;                 //turn everything
            LeftMotorSpeed = MotorNeutral; //everything
            RightMotorSpeed = MotorNeutral; //off
          }}
      }
